...
function App() {
    ...
    const defaultScrubWords = [ // Should match har_sanitize.tsx
        "Authorization",
        "SAMLRequest",
        "SAMLResponse",
        "access_token",
    ...
    const [settings, setSettings] = useState({
        ...
        scrubWords: defaultScrubWords  
    });
    const [newWord, setNewWord] = useState('');
    const [errorMessage, setErrorMessage] = useState('');
    const [specificHeaders, setSpecificHeaders] = useState([]);

    useEffect(() => {
        const fetchSettings = async () => {
            const fetchedSettings = await invoke('getSettings');
            setSettings({
                ...fetchedSettings,
                scrubWords: fetchedSettings.scrubWords.length > 0 ? fetchedSettings.scrubWords : defaultScrubWords
            });
        };
    ...
    const addWord = async () => {
        ...
        if (settings.scrubWords.includes(newWord)) {
            setErrorMessage('This word is already in the list.');
            return;
        }
        const updatedScrubWords = [...settings.scrubWords, newWord].sort();
        const updatedSettings = {
            ...settings,
            scrubWords: updatedScrubWords
        };
        setSettings(updatedSettings);
        await invoke('setSettings', updatedSettings);
        setNewWord('');
        setErrorMessage('');
    };

    const removeWord = async (wordToRemove) => {
        const updatedScrubWords = settings.scrubWords.filter(word => word !== wordToRemove);
        const updatedSettings = {
            ...settings,
            scrubWords: updatedScrubWords
        };
    ...
    const resetScrubWordsToDefault = async () => {
        const updatedSettings = {
            ...settings,
            scrubWords: defaultScrubWords
        };
        ...
        <Page>
            <Grid>
                <GridColumn medium={8}>
                    ...
                    />        
                    <Heading level="h600">Manage Scrubed Words</Heading>
                    <div style={{
                        display: 'flex',
                        flexWrap: 'wrap',
                        ...
                        {settings.scrubWords.map((word) => (
                            <Tag 
                                key={word} // Use the word itself as the key
                                text={word}
                                removeButtonText="Remove"
                                onAfterRemoveAction={() => removeWord(word)}
                            />
                        ))}
                    </div>
                    <div style={{ marginBottom: token('space.500', '40px') }}>
                        ...
                        <Button 
                            appearance="warning" 
                            onClick={resetScrubWordsToDefault}
